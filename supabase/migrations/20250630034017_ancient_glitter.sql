-- =====================================================
-- MAILFLOW - STRUCTURE MYSQL COMPLÈTE
...
-- Procédure pour obtenir l'activité d'un utilisateur
CREATE PROCEDURE `GetUserActivity`(IN user_id VARCHAR(36), IN limit_count INT)
BEGIN
    SELECT 
        al.action,
        al.module,
...
-- Trigger pour logger les modifications des courriers entrants
CREATE TRIGGER `incoming_mails_audit` 
AFTER UPDATE ON `incoming_mails`
FOR EACH ROW
BEGIN
    INSERT INTO activity_logs (user_id, action, module, details, created_at)
    VALUES (
        NEW.updated_by,
        'UPDATE',
        'incoming_mails',
        JSON_OBJECT(
            ...
            'changes', JSON_OBJECT(
                ...
                'old_processed', OLD.is_processed,
                'new_processed', NEW.is_processed
            )
        ),
        NOW()
    );
...
-- Trigger pour logger les modifications des courriers sortants
CREATE TRIGGER `outgoing_mails_audit` 
AFTER UPDATE ON `outgoing_mails`
FOR EACH ROW
BEGIN
    INSERT INTO activity_logs (user_id, action, module, details, created_at)
    VALUES (
